...
class Manager:
  def __init__(self, name:str, setting:Setting):
    ...
    self.__NUMBER_OF_TEST_PROCESSOR = ceil(number_of_processor * 0.1)
    number_of_processor -= self.__NUMBER_OF_TEST_PROCESSOR
    self.__NUMBER_OF_VALID_PROCESSOR = ceil(number_of_processor * 0.2)
  ...
  def get_train_set(self, path:Path, Worker = Worker):
    shared_storage = SharedStorageWithLabel(f"{self.__NAME}_train", self.__SETTING)
    shared_storage.init()
  ...
  def get_valid_set(self, path:Path, Worker = Worker):
    shared_storage = SharedStorageWithLabel(f"{self.__NAME}_valid", self.__SETTING)
    shared_storage.init()
    path_provider = PathProviderWithLabel(path)
  ...
  def get_test_set(self, path:Path, Worker = Worker):
    shared_storage = SharedStorage(f"{self.__NAME}_test", self.__SETTING)
    shared_storage.init()
